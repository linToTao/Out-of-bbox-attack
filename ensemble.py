...
if enable_init_latent_vectors:
    latent_shift_biggan = torch.load(latent_vectors_path).to(device).requires_grad_(True)
...
# Create optimizers
opt_ap = torch.optim.SGD([rowPatch], lr=learing_rate, betas=(0.5, 0.999), amsgrad=True)
...
for epoch in range(start_epoch, n_epochs + 1):
    ...
    writer.add_scalar('latent_code_1st_norm', torch.norm(latent_shift_biggan, p=1) / latent_shift_biggan.shape[0],
                      epoch)
    ...
    print(
        "latent code:     :'" + f"norn_inf:{torch.max(torch.abs(latent_shift_biggan)):.4f}; norm_1:{torch.norm(latent_shift_biggan, p=1) / latent_shift_biggan.shape[0]:.4f}")
