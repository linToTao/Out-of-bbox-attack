...
def save_samples(index, patches, sample_dir, show=False):
    for id, patch in enumerate(patches):
        fake_images = patch.cpu()
        vector_fname = 'generated-vector{1}-{0:0=4d}.pt'.format(index, id)
        torch.save(fake_images, os.path.join(sample_dir, vector_fname))
        print('Saving', vector_fname)
        fake_fname = 'generated-images{1}-{0:0=4d}.png'.format(index, id)
...
def save_samples_GANLatentDiscovery(method_num,
    ...
    vector_fname = 'generated-vector-{0:0=4d}.pt'.format(index)
    torch.save(latent_shift, os.path.join(sample_dir, vector_fname))
    print('Saving', vector_fname)
    fake_fname = 'generated-images-{0:0=4d}.png'.format(index)
    save_image(fake_images, os.path.join(sample_dir, fake_fname), nrow=8)
    print('Saving', fake_fname)
