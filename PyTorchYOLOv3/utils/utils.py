...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, multi_label=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        y = torch.cat((x[:, 0:5], x[:, 5:] * x[:, 4:5]), 1)  # conf = obj_conf * cls_conf
        ...
        #     x = torch.cat((box[i], x[i, 4, None], x[i, j + 5, None], j[:, None].float()), 1)
        ...
        # Filter by class
        if classes is not None:
            y = y[(y[:, 6:7] == torch.tensor(classes, device=x.device)).any(1)]
        ...
        if not n:  # no boxes
            ...
            # sort by confidence
            y = y[y[:, 4].argsort(descending=True)[:max_nms]]

        # Batched NMS
        c = y[:, 5:6] * max_wh  # classes
        # boxes (offset by class), scores
        boxes, scores = y[:, :4], y[:, 5]
